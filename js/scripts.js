...
const GAME_CONFIG = {
    ...
    KeeptalkingAndNobodyHacked: {
        name: "Keep Talking...", image: "gameicons/KeepTalking.png", 
        // ИСПРАВЛЕНО ЗДЕСЬ: заглавная буква T в слове KeepTalking
        url: "KeepTalkingAndNobodyHacked/index.html",
    ...
    NoIAMNotAHacker: {
        name: "Not a Hacker", image: "gameicons/NoH.png", url: "NoIAmNotAHacker/index.html",
    ...
    SecurityAwarnessOS: {
        name: "Security OS", image: "gameicons/SecOS.png", url: "SecurityAwarnessOS/index.html",
...
async function checkAuth() {
    ...
    try {
        ...
        if (res.ok) {
            ...
            document.getElementById('greeting-text').textContent = `Добро пожаловать, ${data.username}!`;
...
function calculateAndRenderProgress() {
    ...
    for (const [gameId, config] of Object.entries(GAME_CONFIG)) {
                ...
                <div class="ach-card ${isUnlocked ? '' : 'locked'}">
                    ...
                    <div class="ach-info">
                        ...
                        <div class="progress-wrap">
                            ...
                            <div class="progress-bar"><div class="progress-fill ${isUnlocked ? 'glow-bg' : ''}" style="width: ${percent}%"></div></div>
                        </div>
                    </div>
                </div>
            `);
        ...
        gamesHtml.push(`
            <div class="steam-card">
                ...
                <div class="steam-card-footer">
                    ...
                    <div class="progress-wrap">
                        <div class="progress-header"><span>Выполнено</span><span class="text-accent">${gamePercent}%</span></div>
                        <div class="progress-bar"><div class="progress-fill glow-bg" style="width: ${gamePercent}%"></div></div>
                    </div>
                </div>
            </div>
        `);
        ...
        profileBarsHtml.push(`
            <div class="progress-wrap" style="background: rgba(0,0,0,0.2); padding: 15px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.05);">
                <div class="progress-header"><span style="color: #fff; font-weight: bold;">${config.name}</span><span class="text-accent">${gamePercent}%</span></div>
                <div class="progress-bar" style="height: 8px;"><div class="progress-fill glow-bg" style="width: ${gamePercent}%"></div></div>
            </div>
        `);
    }
    ...
    // Безопасное обновление элементов (с проверкой на их существование в HTML)
    const overallProgressText = document.getElementById('overall-progress-text');
    if(overallProgressText) overallProgressText.innerText = `${overallPercent}%`;
    
    const overallProgressBar = document.getElementById('overall-progress-bar');
    if(overallProgressBar) overallProgressBar.style.width = `${overallPercent}%`;
    
    const homeScore = document.getElementById('home-score');
    if(homeScore) homeScore.innerText = totalScore;
    
    const homeAchs = document.getElementById('home-achs');
    if(homeAchs) homeAchs.innerText = unlockedAchs;
    ...
    const profileGamesStats = document.getElementById('profile-games-stats');
    if(profileGamesStats) profileGamesStats.innerHTML = profileBarsHtml.join('');
}
...
async function loadLeaderboard() {
    ...
    container.innerHTML = '<div style="text-align: center; padding: 50px; font-size: 1.2em; color: var(--text-muted);">Синхронизация данных...</div>';
...
function renderLeaderboard(data) {
    ...
    if (!data || data.length === 0) {
        container.innerHTML = '<div style="text-align: center; padding: 50px; font-size: 1.2em;">Рейтинг пока пуст. Стань первым!</div>';
        return;
    }
    ...
    let tableHtml = '<table class="lb-table slide-up" style="animation-delay: 0.3s"><thead><tr><th>Rank</th><th>Агент</th><th>Очки</th><th>Награды</th><th>Сессии</th></tr></thead><tbody>';
    data.forEach((user, index) => {
        tableHtml += `
            <tr>
                ...
                <td><span style="color:var(--success); font-weight:bold;">${user.achCount}</span></td>
                <td style="color:var(--text-muted);">${user.runs}</td>
            </tr>
        `;
    });
...
function switchTab(targetTabId) {
    ...
    document.querySelectorAll('.sidebar .menu-link, .main-nav .menu-link').forEach(link => {
        link.classList.remove('active');
        if (link.getAttribute('data-tab') === targetTabId) link.classList.add('active');
    });

    if (targetTabId === 'tab-leaderboard') loadLeaderboard();
    // Запуск рендера графиков
    if (targetTabId === 'tab-profile') renderAnalyticsCharts(userStatsMap); 
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('login-form').addEventListener('submit', async (e) => {
        ...
        err.innerText = 'Установка соединения...';
        try {
            ...
            err.style.color = 'var(--danger)';
            err.innerText = 'Ошибка сети. Сервер недоступен.'; 
        }
    });
    ...
    document.getElementById('register-form').addEventListener('submit', async (e) => {
        ...
        err.style.color = 'var(--text-muted)';
        err.innerText = 'Создание профиля агента...';
        ...
        try {
            ...
            if (res.ok) { 
                ...
                err.innerText = 'Успешно! Инициализация...'; 
    ...
    document.querySelectorAll('.menu-link:not(.dummy)').forEach(trigger => {
...
function renderAnalyticsCharts(statsMap) {
    // Настройки темы графиков
    Chart.defaults.color = '#8f98a0';
    Chart.defaults.font.family = "'Segoe UI', system-ui, sans-serif";

    // 1. Радар компетенций (Radar Chart)
    const ctxRadar = document.getElementById('radarChart');
    if (ctxRadar) {
        if(chartsInstance.radar) chartsInstance.radar.destroy();
        
        // Маппинг данных из базы по модулям
        const att = statsMap['CleanDesk']?.best_score || 0;
        const phish = statsMap['CyberInspector']?.best_score || 0;
        const pass = statsMap['SecurityAwarnessOS']?.best_score || 0;
        const net = statsMap['NoIAMNotAHacker']?.best_score || 0;
        const mem = statsMap['RememberMe']?.best_score || 0;

        chartsInstance.radar = new Chart(ctxRadar.getContext('2d'), {
            type: 'radar',
            data: {
                labels: ['Внимательность', 'Анти-Фишинг', 'Пароли', 'Сети', 'Инциденты'],
                datasets: [{
                    label: 'Уровень навыка (%)',
                    data: [
                        Math.max(20, att), Math.max(20, phish), 
                        Math.max(20, pass), Math.max(20, net), Math.max(20, mem)
                    ],
            ...
            options: {
                scales: {
                    r: { angleLines: { color: 'rgba(255,255,255,0.1)' }, grid: { color: 'rgba(255,255,255,0.1)' }, ticks: { display: false, max: 100, min: 0 } }
                },
    ...
    // 2. Диаграмма уязвимостей (Pie Chart)
    const ctxPie = document.getElementById('pieChart');
    if (ctxPie) {
        if(chartsInstance.pie) chartsInstance.pie.destroy();
        // Здесь можно привязать реальные метрики ошибок из базы
        let errors = [45, 30, 25]; 
        chartsInstance.pie = new Chart(ctxPie.getContext('2d'), {
            type: 'doughnut',
            data: {
                labels: ['Невнимательность', 'Доверие к СИ', 'Спешка'],
                datasets: [{
                    data: errors,
                    backgroundColor: ['#ef4444', '#ffcc00', '#1999ff'],
                    borderWidth: 0,
                    hoverOffset: 10
                }]
            },
    ...
    // 3. Динамика успеваемости (Line Chart)
    const ctxLine = document.getElementById('lineChart');
    if (ctxLine) {
        if(chartsInstance.line) chartsInstance.line.destroy();
        chartsInstance.line = new Chart(ctxLine.getContext('2d'), {
            type: 'line',
            data: {
                labels: ['Сессия 1', 'Сессия 2', 'Сессия 3', 'Сессия 4', 'Сессия 5'],
                datasets: [{
                    label: 'Общий счет',
                    data: [100, 350, 420, 800, 1450], // Эмуляция истории роста
                    borderColor: '#5ba32b',
                    backgroundColor: 'rgba(91, 163, 43, 0.1)',
                    fill: true,
                    tension: 0.4
                }]
            },
