...
const PROTOCOL_RULES = [
    ...
    },
    {
        ...
        check: (req) => req.type === 'WEB' && req.webData.sslExpiry < '1998-01-01'
    }
];
...
const GUIDE_PAGES = [
    ...
    },
    {
        ...
        content: `
            ...
            <div class="guide-entry"><div class="guide-term">WEB REQUEST</div><div class="guide-desc">Смотрите SSL сертификат. Он не должен быть просрочен (Текущий год: 1998).</div></div>
        `
    },
    {
        ...
        content: `
            ...
            <div class="guide-entry"><div class="guide-term">IP ADDRESS</div><div class="guide-desc">Локальная сеть: 192.168.x.x. Остальные требуют проверки протокола.</div></div>
        `
    }
];
...
class RequestGenerator {
    static generate(difficulty) {
        ...
        req.idCard = {
            ...
            expiry: (1999 + Math.floor(Math.random()*5)) + "-12-01",
            org: domain.split('.')[0].toUpperCase() + " CORP"
        };
        if (isAttack && Math.random() > 0.8) req.idCard.expiry = "1995-01-01";
        ...
        if (type === 'WEB') {
            ...
            req.webData = {
                ...
                sslIssuer: Math.random() > 0.5 ? "VeriSign" : "Unknown CA",
                sslExpiry: isAttack ? "1997-05-20" : "1999-05-20"
            };
            req.reason = isAttack ? "EXPIRED_CERT" : null;
        } else if (type === 'DEVICE') {
...
class Game {
    ...
    loadRequest(req) {
        ...
        const isExpired = req.idCard.expiry < '1998-01-01';
        let specificInfo = "";
        ...
        if (req.type === 'WEB') {
            specificInfo = `
                <div class="web-block">
                    ...
                    <div>SSL EXPIRY: ${req.webData.sslExpiry}</div>
                </div>`;
        ...
        viewer.innerHTML = `
            ...
            <div class="id-card-container ${isExpired ? 'fake-id' : ''}">
                ...
                <div class="id-info">
                    ...
                    <div class="id-row"><span class="id-label">ORG:</span> <span class="id-val">${req.idCard.org}</span></div>
                </div>
            </div>
