...
class CampaignManager {
    ...
    playIntro() {
        ...
        if (!ch) { 
            ...
            // Сохранение ачивки за прохождение кампании в новую систему
            try {
                let localStats = JSON.parse(localStorage.getItem('sys_local_stats')) || {};
                if (!localStats['RememberMe']) localStats['RememberMe'] = {};
                if (!localStats['RememberMe'].achievements) localStats['RememberMe'].achievements = [];
                if (!localStats['RememberMe'].achievements.includes('story_cleared')) {
                    localStats['RememberMe'].achievements.push('story_cleared');
                }
                localStorage.setItem('sys_local_stats', JSON.stringify(localStats));
                if (window.saveProgress) window.saveProgress();
            } catch(e) {}

            Engine.modeManager.showMenu(); 
            return; 
        }
...
/* ==========================================================================
   ИГРОВОЙ ДВИЖОК И ИНТЕГРАЦИЯ ПРОГРЕССА
   ========================================================================== */
...
const Engine = {
    ...
    submitAnswer(override) {
        ...
        if (isCorrect) {
            this.stats.correct++;
            if (window.saveProgress) window.saveProgress(); // Синхронизация прогресса
...
// Глобальная функция сохранения новой статистики
window.saveProgress = function() {
    try {
        let localStats = JSON.parse(localStorage.getItem('sys_local_stats')) || {};
        if (!localStats['RememberMe']) localStats['RememberMe'] = {};
        let gameStats = localStats['RememberMe'];

        // Очки за правильные ответы (каждый верный ответ = 50 очков)
        let currentScore = Engine.stats.correct * 50; 
        
        // Обновляем лучший счет
        gameStats.score = Math.max(gameStats.score || 0, currentScore);
        
        // Засчитываем попытку (если игра началась и есть хотя бы 1 ответ)
        if (Engine.stats.correct > 0 || Engine.stats.errors > 0) {
            gameStats.add_run = true;
        }

        // Достижение за хорошую память
        if (!gameStats.achievements) gameStats.achievements = [];
        if (Engine.stats.correct >= 15) {
            if (!gameStats.achievements.includes('memory_master')) {
                gameStats.achievements.push('memory_master');
            }
        }
